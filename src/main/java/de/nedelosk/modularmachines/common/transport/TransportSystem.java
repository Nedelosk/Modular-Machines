...
public class TransportSystem implements ITransportSystem {
	...
	private AdvancedBlockPos referenceCoord;
	...
	public boolean hasBlock(AdvancedBlockPos blockCoord) {
	...
	public void attachPart(ITransportPart part) {
		...
		AdvancedBlockPos coord = part.getWorldLocation();
	...
	public void detachPart(ITransportPart part, boolean chunkUnloading) {
		...
		if (!connectedParts.remove(part)) {
			Log.warn(
					...
					worldObj.isRemote ? "CLIENT" : "SERVER", part.hashCode(), part.getTileEntity().getPos().getX(),
							part.getTileEntity().getPos().getY(), part.getTileEntity().getPos().getZ());
		}
	...
	public String getPartsListString() {
		...
		for(ITransportPart part : connectedParts) {
			...
			sb.append(
					String.format("(%d: %d, %d, %d)", part.hashCode(), part.getTileEntity().getPos()));
			first = false;
		}
		return sb.toString();
	}
	...
	public void auditParts() {
		...
		for(ITransportPart part : connectedParts) {
			if (part.getTileEntity().isInvalid()
					|| worldObj.getTileEntity(part.getTileEntity().getPos()) != part.getTileEntity()) {
				onDetachPart(part);
				deadParts.add(part);
			}
		}
	...
	public Set<ITransportPart> checkForDisconnections() {
		...
		for(ITransportPart part : connectedParts) {
			...
			if (chunkProvider.getLoadedChunk(part.getTileEntity().getPos().getX() >> 4, part.getTileEntity().getPos().getZ() >> 4) == null || part.getTileEntity().isInvalid()) {
				deadParts.add(part);
				onDetachPart(part);
				continue;
			}
			if (worldObj.getTileEntity(part.getTileEntity().getPos()) != part.getTileEntity()) {
				deadParts.add(part);
				onDetachPart(part);
				continue;
			}
	...
	@Override
	public AdvancedBlockPos getReferenceCoord() {
		if (referenceCoord == null) {
			selectNewReferenceCoord();
		}
	...
	private void selectNewReferenceCoord() {
		...
		referenceCoord = null;
		for(ITransportPart part : connectedParts) {
			if (part.getTileEntity().isInvalid() || chunkProvider.getLoadedChunk(part.getTileEntity().getPos().getX() >> 4, part.getTileEntity().getPos().getZ() >> 4) == null) {
	...
	public Set<ITransportPart> detachAllBlocks() {
		...
		IChunkProvider chunkProvider = worldObj.getChunkProvider();
		for(ITransportPart part : connectedParts) {
			if (chunkProvider.getLoadedChunk(part.getTileEntity().getPos().getX() >> 4, part.getTileEntity().getPos().getZ() >> 4) != null) {
				onDetachPart(part);
			}
		}
