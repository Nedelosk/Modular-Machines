...
public class TechTreeCategories {
	...
	public static String getCategoryName(String key) {
		if (TechTreeApi.languageManager.getTranslateData(key) != null) {
			return TechTreeApi.languageManager.getTranslateData(getEntryList(key)).getCatogory();
		}
	...
	public static TechTreeEntry getEntry(String key) {
		...
		for ( Object cat : rc ) {
			...
			for ( Object ri : rl ) {
				if (((TechTreeEntry) ri).key.equals(key)) {
					return (TechTreeEntry) ri;
				}
			}
		}
		return null;
