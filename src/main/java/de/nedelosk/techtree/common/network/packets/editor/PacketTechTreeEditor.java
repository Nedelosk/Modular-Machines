package de.nedelosk.techtree.common.network.packets.editor;
...
public class PacketTechTreeEditor implements IMessage, IMessageHandler<PacketTechTreeEditor, IMessage> {
	...
	public IMessage onMessage(PacketTechTreeEditor message, MessageContext ctx) {
		try {
			File categoryFile = new File(TechTree.configFolder + "/techtree/categorys", message.entry.category);
	...
	public void fromBytes(ByteBuf buf) {
		entry = new TechTreeEntry(ByteBufUtils.readUTF8String(buf), ByteBufUtils.readUTF8String(buf), buf.readInt(), TechPointTypes.values()[buf.readInt()],
				buf.readInt(), buf.readInt(), ByteBufUtils.readItemStack(buf));
		if (buf.readBoolean()) {
			entry.isAutoUnlock();
		}
		if (buf.readBoolean()) {
