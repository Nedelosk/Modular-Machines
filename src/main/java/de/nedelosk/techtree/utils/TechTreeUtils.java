package de.nedelosk.techtree.utils;
...
public class TechTreeUtils {
	...
	public static final Gson GSON = new GsonBuilder().setPrettyPrinting().registerTypeAdapter(TechTreeEntry.class, new TechTreeEntryUtils.Deserializer())
			.registerTypeAdapter(TechTreeEntry.class, new TechTreeEntryUtils.Serializer())
			.registerTypeAdapter(TechTreeCategoryList.class, new TechTreeCategoryUtils.Deserializer())
			.registerTypeAdapter(TechTreeCategoryList.class, new TechTreeCategoryUtils.Serializer()).create();
	public static HashMap<String, String> pages = new HashMap<String, String>();

	public static void readTechPoints() {
		try {
			File file = new File(TechTree.configFolder + "/techtree", "techpoints.json");
			if (!file.exists()) {
				file.createNewFile();
			}
			...
			if (object.has("points") && object.get("points").isJsonArray()) {
				...
				while (iterator.hasNext()) {
					...
					TechTreeApi.addTechPointsToItem(stack, typeList.toArray(new TechPointStack[typeList.size()]));
				}
			}
		} catch (Exception e) {
		}
		...
		public static class Serializer implements JsonSerializer<TechTreeEntry> {
			...
			public JsonElement writeEntry(TechTreeEntry entry) {
				...
				if (entry.icon_item != null) {
					object.add("iconItem", JsonUtils.writeItemElement(entry.icon_item));
			...
			public JsonElement parseRecipe(IRecipe recipe) {
				return JsonUtils.writeItemElement(recipe.getRecipeOutput());
			}
			...
			public JsonElement writePage(TechTreePage page) {
				...
				switch (page.type) {
					...
					case TEXT_CONCEALED:
						...
						break;
					case SMELTING:
						object.add("input", JsonUtils.writeItemElement((ItemStack) page.recipe));
						type = "smelting";
						break;
					case NORMAL_CRAFTING:
						object.add("output", JsonUtils.writeItemElement(page.recipeOutput));
						type = "crafting";
						break;
					default:
						object.addProperty("text", page.text);
