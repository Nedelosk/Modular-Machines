...
import de.nedelosk.forestcore.utils.ItemUtil;
...
import de.nedelosk.techtree.api.TechTreePage.PageType;
import de.nedelosk.techtree.api.crafting.IModularCraftingRecipe;
import de.nedelosk.techtree.api.crafting.ShapedModularCraftingRecipe;
import de.nedelosk.techtree.api.crafting.ShapelessModularCraftingRecipe;
...
public class GuiTechTreePage extends GuiScreen {
	...
	protected void genPageBackground(int par1, int par2, float par3) {
		...
		RenderUtil.bindTexture(new ResourceLocation(tex1));
		GL11.glPushMatrix();
		...
		if (this.tooltip != null) {
			RenderUtil.renderTooltip(((Integer) this.tooltip[1]).intValue(), ((Integer) this.tooltip[2]).intValue(), (List) this.tooltip[0],
					((Integer) this.tooltip[3]).intValue());
			// UtilsFX.drawCustomTooltip(this, itemRenderer,
			// this.fontRendererObj, (List)this.tooltip[0],
			// ((Integer)this.tooltip[1]).intValue(),
			// ((Integer)this.tooltip[2]).intValue(),
			// ((Integer)this.tooltip[3]).intValue());
		}
		RenderUtil.bindTexture(new ResourceLocation(tex1));
		int mx1 = par1 - (sw + 261);
	...
	private void drawModularCraftingPage(int side, int x, int y, int mx, int my, TechTreePage pageParm) {
		...
		GL11.glPushMatrix();
		int start = side * 152;
		RenderUtil.bindTexture(new ResourceLocation(tex2));
		GL11.glPushMatrix();
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		GL11.glEnable(3042);
		...
		if ((mposx >= x + 48 + start) && (mposy >= y + 32) && (mposx < x + 48 + start + 16) && (mposy < y + 32 + 16)) {
			drawCustomTooltip(this, itemRenderer, this.fontRendererObj,
					recipe.getRecipeOutput().getTooltip(this.mc.thePlayer, this.mc.gameSettings.advancedItemTooltips), mx, my, 11);
		}
		if ((recipe != null) && (((recipe instanceof ShapedModularCraftingRecipe)))) {
			...
			for ( int i = 0; (i < rw) && (i < 3); i++ ) {
				for ( int j = 0; (j < rh) && (j < 3); j++ ) {
					if (items[(i + j * rw)] != null) {
						...
						GL11.glEnable(2884);
						GL11.glTranslated(0.0D, 0.0D, 100.0D);
						itemRenderer.renderItemAndEffectIntoGUI(this.mc.fontRenderer, this.mc.renderEngine, (ItemStack) items[(i + j * rw)],
								x + start + 16 + i * 32, y + 76 + j * 32);
						itemRenderer.renderItemOverlayIntoGUI(this.mc.fontRenderer, this.mc.renderEngine,
								((ItemStack) items[(i + j * rw)]).copy().splitStack(1), x + start + 16 + i * 32, y + 76 + j * 32);
						RenderHelper.disableStandardItemLighting();
						GL11.glEnable(2896);
						GL11.glPopMatrix();
					}
			...
			for ( int i = 0; (i < rw) && (i < 3); i++ ) {
				for ( int j = 0; (j < rh) && (j < 3); j++ ) {
					if (items[(i + j * rw)] != null) {
						if ((mposx >= x + 16 + start + i * 32) && (mposy >= y + 76 + j * 32) && (mposx < x + 16 + start + i * 32 + 16)
								&& (mposy < y + 76 + j * 32 + 16)) {
							...
							if ((ref != null) && (!((String) ref[0]).equals(this.entry.key))) {
								...
								this.reference
										.add(Arrays.asList(new Serializable[] { Integer.valueOf(mx), Integer.valueOf(my), (String) ref[0], (Integer) ref[1] }));
							}
		...
		if ((recipe != null) && (((recipe instanceof ShapelessModularCraftingRecipe)))) {
			...
			for ( int i = 0; (i < items.size()) && (i < 9); i++ ) {
				if (items.get(i) != null) {
					...
					GL11.glEnable(2884);
					GL11.glTranslated(0.0D, 0.0D, 100.0D);
					itemRenderer.renderItemAndEffectIntoGUI(this.mc.fontRenderer, this.mc.renderEngine, (ItemStack) items.get(i), x + start + 16 + i % 3 * 32,
							y + 76 + i / 3 * 32);
					itemRenderer.renderItemOverlayIntoGUI(this.mc.fontRenderer, this.mc.renderEngine, ((ItemStack) items.get(i)).copy().splitStack(1),
							x + start + 16 + i % 3 * 32, y + 76 + i / 3 * 32);
					RenderHelper.disableStandardItemLighting();
					GL11.glEnable(2896);
					GL11.glPopMatrix();
				}
			...
			for ( int i = 0; (i < items.size()) && (i < 9); i++ ) {
				if (items.get(i) != null) {
					if ((mposx >= x + 16 + start + i % 3 * 32) && (mposy >= y + 76 + i / 3 * 32) && (mposx < x + 16 + start + i % 3 * 32 + 16)
							&& (mposy < y + 76 + i / 3 * 32 + 16)) {
			...
			for ( int i = 0; (i < rw) && (i < 3); i++ ) {
				for ( int j = 0; (j < rh) && (j < 3); j++ ) {
					if (items[(i + j * rw)] != null) {
						...
						GL11.glEnable(2884);
						GL11.glTranslated(0.0D, 0.0D, 100.0D);
						itemRenderer.renderItemAndEffectIntoGUI(this.mc.fontRenderer, this.mc.renderEngine, ItemUtil.cycleItemStack(items[(i + j * rw)]),
								x + start + 16 + i * 32, y + 76 + j * 32);
						itemRenderer.renderItemOverlayIntoGUI(this.mc.fontRenderer, this.mc.renderEngine,
								(ItemUtil.cycleItemStack(items[(i + j * rw)])).copy().splitStack(1), x + start + 16 + i * 32, y + 76 + j * 32);
						RenderHelper.disableStandardItemLighting();
						GL11.glEnable(2896);
						GL11.glPopMatrix();
					}
			...
			for ( int i = 0; (i < rw) && (i < 3); i++ ) {
				for ( int j = 0; (j < rh) && (j < 3); j++ ) {
					if (items[(i + j * rw)] != null) {
						if ((mposx >= x + 16 + start + i * 32) && (mposy >= y + 76 + j * 32) && (mposx < x + 16 + start + i * 32 + 16)
								&& (mposy < y + 76 + j * 32 + 16)) {
							List addtext = (ItemUtil.cycleItemStack(items[(i + j * rw)])).getTooltip(this.mc.thePlayer,
									this.mc.gameSettings.advancedItemTooltips);
							Object[] ref = findRecipeReference((ItemUtil.cycleItemStack(items[(i + j * rw)])));
							if ((ref != null) && (!((String) ref[0]).equals(this.entry.key))) {
								addtext.add("§8§o" + StatCollector.translateToLocal("recipe.clickthrough"));
								this.reference
										.add(Arrays.asList(new Serializable[] { Integer.valueOf(mx), Integer.valueOf(my), (String) ref[0], (Integer) ref[1] }));
		...
		if ((recipe != null) && (((recipe instanceof ShapelessRecipes)) || ((recipe instanceof ShapelessOreRecipe)))) {
			...
			for ( int i = 0; (i < items.size()) && (i < 9); i++ ) {
				if (items.get(i) != null) {
					...
					GL11.glEnable(2884);
					GL11.glTranslated(0.0D, 0.0D, 100.0D);
					itemRenderer.renderItemAndEffectIntoGUI(this.mc.fontRenderer, this.mc.renderEngine, ItemUtil.cycleItemStack(items.get(i)),
							x + start + 16 + i % 3 * 32, y + 76 + i / 3 * 32);
					itemRenderer.renderItemOverlayIntoGUI(this.mc.fontRenderer, this.mc.renderEngine,
							(ItemUtil.cycleItemStack(items.get(i))).copy().splitStack(1), x + start + 16 + i % 3 * 32, y + 76 + i / 3 * 32);
					RenderHelper.disableStandardItemLighting();
					GL11.glEnable(2896);
					GL11.glPopMatrix();
				}
			...
			for ( int i = 0; (i < items.size()) && (i < 9); i++ ) {
				if (items.get(i) != null) {
					if ((mposx >= x + 16 + start + i % 3 * 32) && (mposy >= y + 76 + i / 3 * 32) && (mposx < x + 16 + start + i % 3 * 32 + 16)
							&& (mposy < y + 76 + i / 3 * 32 + 16)) {
						List addtext = (ItemUtil.cycleItemStack(items.get(i))).getTooltip(this.mc.thePlayer, this.mc.gameSettings.advancedItemTooltips);
						Object[] ref = findRecipeReference((ItemUtil.cycleItemStack(items.get(i))));
						if ((ref != null) && (!((String) ref[0]).equals(this.entry.key))) {
							addtext.add("§8§o" + StatCollector.translateToLocal("recipe.clickthrough"));
							this.reference
									.add(Arrays.asList(new Serializable[] { Integer.valueOf(mx), Integer.valueOf(my), (String) ref[0], (Integer) ref[1] }));
	...
	private void drawSmeltingPage(int side, int x, int y, int mx, int my, TechTreePage pageParm) {
		...
		if ((in != null) && (out != null)) {
			...
			if ((mposx >= x + 48 + start) && (mposy >= y + 144) && (mposx < x + 48 + start + 16) && (mposy < y + 144 + 16)) {
				drawCustomTooltip(this, itemRenderer, this.fontRendererObj, out.getTooltip(this.mc.thePlayer, this.mc.gameSettings.advancedItemTooltips), mx,
						my, 11);
			}
			GL11.glPopMatrix();
		}
	...
	private void drawTextPage(int side, int x, int y, String text) {
		...
		RenderHelper.enableGUIStandardItemLighting();
		GL11.glEnable(3042);
		RenderUtil.renderText(x - 15 + start, y - 10, 152, this.height, text);
		// TechTree.fr.drawSplitString(text, x - 15 + side * 152, y, 139, 0,
		// this);
		GL11.glPopMatrix();
	}
	...
	protected void mouseClicked(int par1, int par2, int par3) {
		...
		if ((this.page < this.maxPages - 2) && (mx >= 0) && (my >= 0) && (mx < 14) && (my < 10)) {
			...
			this.lastCycle = 0L;
			this.cycle = -1;
			Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.posX, Minecraft.getMinecraft().thePlayer.posY,
					Minecraft.getMinecraft().thePlayer.posZ, "thaumcraft:page", 0.66F, 1.0F, false);
		}
		mx = par1 - (var4 - 17);
		my = par2 - (var5 + 189);
		...
		if (!history.isEmpty()) {
			...
			if ((mx >= 0) && (my >= 0) && (mx < 20) && (my < 12)) {
				...
				this.mc.displayGuiScreen(
						new GuiTechTreePage(TechTreeCategories.getEntry((String) o[0]), ((Integer) o[1]).intValue(), this.guiMapX, this.guiMapY));
			}
		}
		if (this.reference.size() > 0) {
			for ( List coords : this.reference ) {
				if ((par1 >= ((Integer) coords.get(0)).intValue()) && (par2 >= ((Integer) coords.get(1)).intValue())
						&& (par1 < ((Integer) coords.get(0)).intValue() + 16) && (par2 < ((Integer) coords.get(1)).intValue() + 16)) {
					Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.posX, Minecraft.getMinecraft().thePlayer.posY,
							Minecraft.getMinecraft().thePlayer.posZ, "thaumcraft:page", 0.66F, 1.0F, false);
					history.push(new Object[] { this.entry.key, Integer.valueOf(this.page) });
					this.mc.displayGuiScreen(new GuiTechTreePage(TechTreeCategories.getEntry((String) coords.get(2)), ((Integer) coords.get(3)).intValue(),
							this.guiMapX, this.guiMapY));
				}
			}
		}
