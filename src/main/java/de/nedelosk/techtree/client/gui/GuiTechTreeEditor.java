...
import de.nedelosk.forestmods.common.inventory.InventoryCraftingEditor;
...
import de.nedelosk.techtree.client.gui.gatgets.GuiButtonSlot;
import de.nedelosk.techtree.client.gui.gatgets.GuiTab;
import de.nedelosk.techtree.client.gui.gatgets.GuiTextBox;
...
import de.nedelosk.techtree.utils.TechTreeEditorData;
import de.nedelosk.techtree.utils.TechTreeEditorUtils.TechTreeEditorLanguage;
import de.nedelosk.techtree.utils.TechTreeUtils.Reader;
import de.nedelosk.techtree.utils.TechTreeUtils.Writer;
import de.nedelosk.techtree.utils.language.LanguageManager;
import de.nedelosk.techtree.utils.language.LanguageManager.TechTreeEntryLanguage;
import de.nedelosk.techtree.utils.language.TechTreeEntryLanguageData;
import de.nedelosk.techtree.utils.language.editor.TechTreeEntryEditorLanguageData;
...
public class GuiTechTreeEditor extends GuiScreen {
	...
	public static final Gson GSON = new GsonBuilder().setPrettyPrinting()
			.registerTypeAdapter(TechTreeEntryEditorLanguageData.class, new TechTreeEntryLanguage.Serializer())
			.registerTypeAdapter(TechTreeEntryEditorLanguageData.class, new TechTreeEditorLanguage.Deserializer()).create();
	...
	public void drawScreen(int mX, int mY, float p_73863_3_) {
		...
		RenderUtil.bindTexture(new ResourceLocation("modularmachines", "textures/gui/gui_techtree_editor.png"));
	...
	public void savePages() {
		if (utils.mode == Mode.PAGES) {
			if (entryPages.get(page).type == PageType.TEXT || entryPages.get(page).type == PageType.TEXT_CONCEALED
					|| entryPages.get(page).type == PageType.IMAGE || entryPages.get(page).type == PageType.NORMAL_CRAFTING
					|| entryPages.get(page).type == PageType.SMELTING) {
				if (data.languagData.get(data.languag) == null) {
					data.languagData.put(data.languag, new TechTreeEntryLanguageData());
				}
				if (data.languagData.get(data.languag).pages == null) {
	...
	public class EditorUtils {
		...
		public GuiTextField fieldKey, fieldCategory, fieldTechPoints, fieldParents, fieldSiblings, fieldColumn, fieldRow, fieldCIcon, fieldCBackground,
				fieldCKey, fieldCKeyLanguag, fieldPageOther = null;
		private GuiButton buttonClose, buttonSave, buttonDelete, buttonLaodFile, buttonLeftPage, buttonRightPage, buttonSwitchType, buttonTechPoint,
				buttonAutoUnlock, buttonConcealed, buttonIcon, buttonSwitchLanguage;
		...
		public void drawScreen() {
			if (mode == Mode.DEFAULT) {
				...
				if (entryPages.get(page).type == PageType.TEXT || entryPages.get(page).type == PageType.TEXT_CONCEALED
						|| entryPages.get(page).type == PageType.IMAGE) {
		...
		public void initGui() {
			...
			buttonList.add(
					buttonTechPoint = new GuiButton(10, left + 6, top + 95, 95, 20, StatCollector.translateToLocal("mm.techpoint." + data.pointType.name())));
			buttonList
					.add(buttonAutoUnlock = new GuiButton(11, left + 144, top + 117, 95, 20, StatCollector.translateToLocal("techtree.editor.not.autounlock")));
	...
	public class Editor {
		...
		public void saveEntry() {
			try {
				TechTreeEntry entry = new TechTreeEntry(utils.fieldKey.getText(), utils.fieldCategory.getText(),
						Integer.parseInt(utils.fieldTechPoints.getText()), data.pointType, Integer.parseInt(utils.fieldColumn.getText()),
						Integer.parseInt(utils.fieldRow.getText()), data.icon_item);
				...
				File categoryFile = new File(TechTree.configFolder + "/techtree/categorys", utils.fieldCategory.getText());
				...
				writerEntry.write(GSON.toJson(new TechTreeEntryEditorLanguageData(
						new TechTreeEntryLanguageData("mm.techtree_entry_name." + entry.key, "mm.techtree_entry_text." + entry.key, pages), data.languagData)));
				writerEntry.close();
		...
		public void loadEntry() {
			try {
				File entryLanguageFile = new File(TechTree.configFolder + "/techtree/categorys/" + utils.fieldCategory.getText(),
						utils.fieldKey.getText().toLowerCase() + "_language.json");
				...
				File entryFile = new File(TechTree.configFolder + "/techtree/categorys/" + utils.fieldCategory.getText(),
						utils.fieldKey.getText().toLowerCase() + ".json");
