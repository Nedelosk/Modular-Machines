...
public class BlockTransport extends BlockContainerForest {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
		...
		if (tile instanceof TileEntityTransport) {
			...
			if (playerStack != null && playerStack.getItem() instanceof IToolWrench) {
				IPartSide partSide = transport.getTransportPart().getSide(side);
				...
				TransportRegistry.onPartAdded(world, transport.getTransportPart());
			}
		}
		return false;
	}
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
		...
		if (md == 0 && tile != null && tile instanceof TileEntityTransport) {
			...
			IPartSide[] sides = port.getTransportPart().getSides();
			double minX;
			double minY;
			double minZ;
			double maxX;
	...
	public void setBlockBoundsBasedOnState(IBlockAccess iblockaccess, int x, int y, int z) {
		...
		if (md == 0 && tile != null && tile instanceof TileEntityTransport) {
			...
			if (sides[0].isActive()) {
				...
				if (iblockaccess.isAirBlock(x, y, z)) {
				} else {
					minY = 0;
				}
			}
