...
import de.nedelosk.forestmods.client.gui.GuiWorkbench;
...
import de.nedelosk.forestmods.common.inventory.ContainerWorkbench;
		...
		if (stackSlotTool != null) {
			if (burnTime < burnTimeTotal) {
				...
				if (recipe != null) {
					...
					burnTime = 0;
					burnTimeTotal = (recipe.getBurnTime() == -1) ? Config.worktableBurnTime : recipe.getBurnTime();
					isWorking = true;
				}
			}
		}
