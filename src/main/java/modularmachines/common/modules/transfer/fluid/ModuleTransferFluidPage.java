...
public class ModuleTransferFluidPage extends ModuleTransferPage<ModuleTransferFluid, IFluidHandler> {
	...
	public ModuleTransferFluidPage(ModuleTransferFluid parent, int index) {
		super(parent, index);
	}
	...
	public void addCycleWidgets() {
		List<ITransferHandlerWrapper<IFluidHandler>> wrappers = parent.getValidWrappers();
		if(!wrappers.isEmpty()){
			int xSize = gui.xSize;
			int cycleSize = 110;
	...
	public void addCycle(){
		if(doAdd){
			...
			PacketHandler.sendToServer(new PacketAddCycle(parent, new FluidTransferCycle(parent, startHandler.getCurrentWrapper(), endHandler.getCurrentWrapper(), time, priority, amount)));
		}
	}
