...
public class ItemBattery extends Item implements IEnergyItem, IItemModelRegister {
	...
	public boolean showDurabilityBar(ItemStack stack) {
		return ItemEnergyStorage.hasEnergy(stack);
	}

	@Override
	public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
		NBTTagCompound tagCombound = stack.getTagCompound();
		if (!ItemEnergyStorage.hasEnergy(tagCombound)) {
			return;
		}
		tooltip.add(Translator.translateToLocal("mm.tooltip.energy") + ItemEnergyStorage.getEnergy(tagCombound) + " / " + getCapacity(stack));
	}
	...
	public double getDurabilityForDisplay(ItemStack stack) {
		...
		double energy = ItemEnergyStorage.getEnergy(tagCombound) * 100 / getCapacity(stack) * 100;
	...
	public void getSubItems(Item itemIn, CreativeTabs tab, List<ItemStack> subItems) {
		for (int i = 0; i < CAPACITY.length; i++) {
			subItems.add(ItemEnergyStorage.createItemStack(this, 1, i, true));
			subItems.add(ItemEnergyStorage.createItemStack(this, 1, i, false));
		}
	}

	@Override
	public ICapabilityProvider initCapabilities(ItemStack stack, NBTTagCompound nbt) {
		return new ItemEnergyStorage(stack, this);
	}
	...
	private static class BatteryMeshDefinition implements ItemMeshDefinition {
		...
		public ModelResourceLocation getModelLocation(ItemStack stack) {
			if (ItemEnergyStorage.getEnergy(stack) >= ItemEnergyStorage.getCapacity(stack)) {
