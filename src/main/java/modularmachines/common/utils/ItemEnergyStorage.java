...
public class ItemEnergyStorage implements IEnergyStorage, ICapabilityProvider {
	...
	public ItemEnergyStorage(ItemStack container, IEnergyItem energyItem) {
		this(container, energyItem, 1000, 1000);
	}

	public ItemEnergyStorage(ItemStack container, IEnergyItem energyItem, int maxTransfer) {
		this(container, energyItem, maxTransfer, maxTransfer);
	}

	public ItemEnergyStorage(ItemStack container, IEnergyItem energyItem, int maxReceive, int maxExtract) {
	...
	public static ItemStack createItemStack(IEnergyItem item, int size, int damage, boolean empty) {
		...
		tagCombound.setInteger(ItemEnergyStorage.ENERGY_KEY, empty ? 0 : item.getCapacity(itemStack));
	...
	public static ItemStack createItemStack(Item item, int size, int damage, int energy) {
		...
		NBTTagCompound tagCombound = new NBTTagCompound();
		tagCombound.setInteger(ItemEnergyStorage.ENERGY_KEY, energy);
		return itemStack;
	}
	...
	public static int getEnergy(NBTTagCompound tagCombound) {
		...
		return tagCombound.getInteger(ItemEnergyStorage.ENERGY_KEY);
	}
	...
	public static boolean hasEnergy(@Nullable NBTTagCompound tagCombound) {
		return tagCombound != null && tagCombound.hasKey(ItemEnergyStorage.ENERGY_KEY);
	}
}
