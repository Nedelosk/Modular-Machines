...
public class WidgetModulePageTab extends Widget<IModulePage> {
	...
	public void draw(IGuiBase gui) {
		...
		gui.getGui().drawTexturedModalRect(gui.getGuiLeft() + positon.x, gui.getGuiTop() + positon.y, source.getModuleState().getModular().getCurrentPage().getPageID().equals(source.getPageID()) ? 74 : 103, isDown ? 237 : 218, 29, 19);
		RenderUtil.bindTexture(new ResourceLocation("modularmachines:textures/gui/widgets.png"));
		gui.getGui().drawTexturedModalRect(gui.getGuiLeft() + positon.x + 6, gui.getGuiTop() + positon.y, 0, 18 + pageIndex * 18, 18, 18);
	...
	public void handleMouseClick(int mouseX, int mouseY, int mouseButton, IGuiBase gui) {
		...
		if (!source.getPageID().equals(source.getModuleState().getModular().getCurrentPage().getPageID())) {
			Minecraft.getMinecraft().getSoundHandler().playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 1.0F));
			modularHandler.getModular().setCurrentPage(source.getPageID());
			PacketHandler.sendToServer(new PacketSelectModulePage(modularHandler, source.getPageID()));
		}
	}

	@Override
	public List getTooltip(IGuiBase gui) {
		return Arrays.asList(source.getTabTitle());
	}
}
