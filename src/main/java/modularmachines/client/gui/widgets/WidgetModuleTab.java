...
public class WidgetModuleTab extends Widget<IModuleState> {
	...
	public void draw(IGuiBase gui) {
		if (gui != null && source != null && moduleHandler != null && moduleHandler.getModular() != null && moduleHandler.getModular().getCurrentModule() != null) {
			GlStateManager.color(1F, 1F, 1F, 1F);
			RenderUtil.bindTexture(guiTexture);
			gui.getGui().drawTexturedModalRect(gui.getGuiLeft() + positon.x, gui.getGuiTop() + positon.y, (source.getPosition() == moduleHandler.getModular().getCurrentModule().getPosition()) ? 0 : 28, right ? 214 : 235, 28, 21);
			gui.drawItemStack(source.getSource().getItemStack(), gui.getGuiLeft() + positon.x + (right ? 5 : 7), gui.getGuiTop() + positon.y + 2);
		}
	}
