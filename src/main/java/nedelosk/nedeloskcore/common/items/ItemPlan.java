...
public class ItemPlan extends Item implements IPlan {
	...
	public ItemPlan() {
		...
		setHasSubtypes(true);
        setUnlocalizedName(NCRegistry.setUnlocalizedItemName("plan", "nc"));
	}
	
	@Override
	public String getUnlocalizedName(ItemStack stack) {
		return NCRegistry.setUnlocalizedItemName("plan", "nc") + "." + stack.getItemDamage();
	}
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z,int p_77648_7_, float p_77648_8_, float p_77648_9_,float p_77648_10_) {
	    ...
	    else if(!world.isRemote)
	    {
	    	...
	    	{
	    	  if(world.getBlock(x, y, z) != null && world.getBlock(x, y, z) != Blocks.air && world.getBlock(x, y, z) != NCBlockManager.Plan_Block.block())
	    	  {
	    		  if(world.getBlock(x, y + 1, z) == Blocks.air)
	    		  {
	    			  if(getUpdateBlock(stack) == null)
	    			  {
	    			  if(!(world.setBlock(x, y + 1, z, NCBlockManager.Plan_Block.block())))
	    					  return  false;
	    			  ...
	    			  world.playSoundEffect(x + 0.5F, y + 0.5F, z + 0.5F, NCBlockManager.Plan_Block.block().stepSound.func_150496_b(), (NCBlockManager.Plan_Block.block().stepSound.getVolume() + 1.0F) / 2.0F, NCBlockManager.Plan_Block.block().stepSound.getPitch() * 0.8F);
		    		...
		    		if(getUpdateBlock(stackInv) != null && world.getBlock(x, y, z) != null && world.getBlock(x, y, z) == Block.getBlockFromItem(getUpdateBlock(stackInv).getItem()) && world.getBlockMetadata(x, y, z) == getUpdateBlock(stackInv).getItemDamage())
		    		{
		  	    	  if(world.getBlock(x, y, z) != null && world.getBlock(x, y, z) != Blocks.air)
			    	  {
		  	    		  NBTTagCompound nbt = stack.getTagCompound().getTagList("slots", 10).getCompoundTagAt(0).getCompoundTag("tag");
			    			  if(!(world.setBlock(x, y, z, NCBlockManager.Plan_Block.block())))
			    					  return  false;
