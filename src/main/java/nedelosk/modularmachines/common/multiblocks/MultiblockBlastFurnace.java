...
public class MultiblockBlastFurnace extends MultiblockModularMachines {
	...
	public FluidTankNedelosk tankSlag;
	public FluidTankNedelosk tank;
	public FluidTankNedelosk tankAirHot;
	...
	public MultiblockPattern createPattern() {
		...
		char[][] top_layer_1 = { 
				...
				{ 'M', 'C', 'C', 'C', 'M'},
				{ 'M', 'M', 'F', 'M', 'M'}};
		return new MultiblockPattern(new char[][][]{ bottom_layer_1, bottom_layer_2, bottom_layer_3, center_layer_1, center_layer_2, center_layer_1, center_layer_1, center_layer_1, top_layer_1}, 2, 0, 2);
		}
	...
	public Object getGUIContainer(TileMultiblockBase tile, InventoryPlayer inventory) {
		return new GuiBlastFurnace(tile, inventory);
	}
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		if(nbt.hasKey("TankAirHot"))
		{
			...
			tankAirHot = new FluidTankNedelosk(nbtTag.getInteger("Capacity"));
		...
		if(nbt.hasKey("TankSlag"))
		{
			...
			tankSlag = new FluidTankNedelosk(nbtTag.getInteger("Capacity"));
	...
	public void updateServer(TileMultiblockBase tile) {
			if(tile.slots == null || tile.slots.length == 0)
				tile.slots = new ItemStack[4];
			if(tankGas == null)
				tankGas = new FluidTankNedelosk(32000);
			if(tankSlag == null)
				tankSlag = new FluidTankNedelosk(32000);
			if(tank == null)
				tank = new FluidTankNedelosk(24000);
			if(tankAirHot == null)
				tankAirHot = new FluidTankNedelosk(32000);
		...
		{
			if(tile.getWorldObj().rand.nextInt(40) == 20)
