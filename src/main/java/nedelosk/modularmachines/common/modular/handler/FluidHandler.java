...
public class FluidHandler implements INBTTagable, IFluidHandler {
	 ...
	 */
	public FluidHandler(ModularMachine machine, FluidTankNedelosk... tanks){
		this.tanks = tanks;
		this.machine = machine;
	}
	
	public FluidHandler(NBTTagCompound nbt, ModularMachine machine){
		readFromNBT(nbt);
		this.machine = machine;
	}
	...
	public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
		if(!doFill)
			return 0;
		ModuleTankManager manager = (ModuleTankManager) machine.getTankManeger();
		if(manager != null)
		{
