...
public class WorldGeneratorModularMachines implements IWorldGenerator {
	...
	private void generateSurface(World world, Random random, int x, int z) {
		if (Config.generateColumbiteOre) {
			generateOre(ModuleCore.BlockManager.Ore_Others.block(), 0, world, random, x, z, 2 + random.nextInt(3), 1, 0, 25);
		}
		if (Config.generateAluminiumOre) {
			generateOre(ModuleCore.BlockManager.Ore_Others.block(), 1, world, random, x, z, 4 + random.nextInt(3), 3, 20, 65);
		}
	}
