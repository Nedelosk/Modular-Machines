...
public class ProducerTankManager extends ProducerManager implements IProducerTankManager {
	...
	public boolean transferInput(ModuleStack<IModule, IProducerInventory> stackModule, IModularTileEntity tile,
		...
		if (stack != null && stack.getModule() == Modules.TANK && stack.getProducer() != null
				&& stack.getProducer() instanceof IProducerTank) {
		...
		if (modular.getManager().getFluidHandler() != null) {
			...
			for (int ID = tab * 3; ID < (tab + 1) * 3; ID++) {
				if (!(datas.length <= ID)) {
					...
					WidgetFluidTank widget = new WidgetFluidTank(data == null ? null : data.getTank(), 36 + i * 51, 18,
							ID);
	...
	public void addWidgets(Widget widget, IGuiBase<IModularTileEntity<IModular>> gui) {
		if (widget != null && widget instanceof WidgetFluidTank) {
			...
			gui.getWidgetManager()
					.add(new WidgetTankMode(tank.posX - 22, tank.posY, data == null ? null : data.getMode(), tank.ID));
			if (gui.getTile().getModular().getFluidProducers() != null
					&& !gui.getTile().getModular().getFluidProducers().isEmpty())
				gui.getWidgetManager().add(new WidgetProducer(tank.posX - 22, tank.posY + 21,
						data == null ? -1 : data.getProducer(), tank.ID));
			gui.getWidgetManager().add(new WidgetDirection(tank.posX - 22, tank.posY + 42,
					data == null ? null : data.getDirection(), tank.ID));
		}
	}
	...
	public void addButtons(IGuiBase gui, IModular modular, ModuleStack stack) {
		...
		for (int ID = 0; ID < tabs; ID++) {
			gui.getButtonManager()
					.add(new ButtonTabTankManager(gui.getButtons().size() + gui.getButtonManager().getButtons().size(),
							ID > 4 ? 12 + gui.getGuiLeft() + (ID - 5) * 30 : 12 + gui.getGuiLeft() + ID * 30,
							ID > 4 ? 196 + gui.getGuiTop() : -19 + gui.getGuiTop(), stack, ID > 4 ? true : false, ID));
		}
	}
	...
	public int fill(ForgeDirection from, FluidStack resource, boolean doFill, ModuleStack stack, IModular modular) {
		...
		for (int i = 0; i < datas.length; i++) {
			...
			if (data.getMode() == TankMode.OUTPUT && stack != null
					|| data.getMode() == TankMode.INPUT && stack == null) {
				if (from == data.getDirection() || from == ForgeDirection.UNKNOWN
						|| data.getDirection() == ForgeDirection.UNKNOWN) {
	...
	@Override
	public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain, ModuleStack stack,
			IModular modular) {
		if (resource == null)
			return null;
		for (int i = 0; i < datas.length; i++) {
	...
	public void addTank(int id, IProducerTank tank) {
		if (tank == null)
			...
			datas[id] = new TankData(new FluidTankSimple(tank.getCapacity()));
	}
	...
	public List<ITankData> getDatas(IModular modular, ModuleStack producer, TankMode mode) {
		...
		if (producer != null) {
			if (modular.getFluidProducers() == null || modular.getFluidProducers().isEmpty()
					|| !modular.getFluidProducers().contains(producer))
				return datasL;
	...
	public class SlotTank extends SlotModular {
		...
		public SlotTank(IInventory p_i1824_1_, int p_i1824_2_, int p_i1824_3_, int p_i1824_4_, ModuleStack stack,
				int ID) {
		...
		public boolean isItemValid(ItemStack stack) {
			ModuleStack stackModule = ModuleRegistry.getProducer(stack);
			if (stackModule != null && stackModule.getModule() != null && stackModule.getProducer() != null
					&& stackModule.getProducer() instanceof IProducerTank)
				return true;
			return false;
		}
		...
		public void onPickupFromSlot(EntityPlayer player, ItemStack stack) {
			...
			ModuleStack module = ModuleRegistry.getProducer(stack);
			if (module != null && module.getModule() != null && module.getProducer() != null
					&& module.getProducer() instanceof IProducerTank) {
