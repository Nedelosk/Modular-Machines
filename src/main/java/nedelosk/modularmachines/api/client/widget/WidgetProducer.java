...
public class WidgetProducer<T extends TileEntity & IModularTileEntity> extends Widget<T> {
	...
	public void draw(IGuiBase<T> gui) {
		...
		if (stack.getItem() != null)
			GuiBase.getItemRenderer().renderItemIntoGUI(mc.fontRenderer, mc.renderEngine, stack.getItem(),
					gui.getGuiLeft() + pos.x + 1, gui.getGuiTop() + pos.y + 1);
	...
	public void handleMouseClick(int mouseX, int mouseY, int mouseButton, IGuiBase<T> gui) {
		if (gui.getTile() != null && gui.getTile().getModular() != null
				&& gui.getTile().getModular().getFluidProducers() != null && producer != -1) {
