...
public class ModuleCore extends AModule {
	...
	public void preInit(IModuleManager manager) {
		...
		manager.register(BlockManager.Ore_Others, new BlockOre(oreOtherOres, "modularmachines"),
				ItemBlockForestDay.class);
		manager.register(BlockManager.Component_Metal_Blocks, new BlockComponent(Material.iron, "metal_block"),
				ItemBlockForestDay.class);
	...
	public void init(IModuleManager manager) {
		...
		for (int i = 0; i < ((BlockComponent) BlockManager.Component_Metal_Blocks.block()).metas.size(); i++) {
			...
			if (((BlockComponent) BlockManager.Component_Metal_Blocks.block()).metas.get(i).get(2) != null)
				for (String oreDict : (String[]) ((BlockComponent) BlockManager.Component_Metal_Blocks.block()).metas
						.get(i).get(2))
	...
	public static void registerFluids() {
		White_Pig_Iron = Registry.registerFluid("white_pig_iron", 1500, Material.lava, true, false).setDensity(3000)
				.setViscosity(6000);
		Gray_Pig_Iron = Registry.registerFluid("gray_pig_iron", 1500, Material.lava, true, false).setDensity(3000)
				.setViscosity(6000);
	...
	public static void registerComponentRecipes() {
		...
		manager.addRecipe(new OreStack("cobblestone", 2), new OreStack("toolHammer"),
				new ItemStack(ItemManager.Component_Plates.item()), 100);
		for (int i = 0; i < ((ItemComponent) ItemManager.Component_Plates.item()).metas.size(); i++) {
			ItemStack stack = new ItemStack(ItemManager.Component_Plates.item(), 1, i);
			ItemComponent component = (ItemComponent) stack.getItem();
			if (component.metas.get(i).get(2) != null)
				...
				for (String oreDict : (String[]) component.metas.get(i).get(2))
					if (!oreDict.equals("Bronze") && !oreDict.equals("Steel"))
						...
						registerRecipe(new RecipeLathe(new RecipeItem(new OreStack("plate" + oreDict)),
								new RecipeItem(new ItemStack(ItemManager.Component_Wire.item(), 8, i)), 1, 250,
								LatheModes.WIRE));
		}
		...
		for (int i = 0; i < ((ItemComponent) ItemManager.Component_Screws.item()).metas.size(); i++) {
			...
			if (component.metas.get(i).get(2) != null)
				for (String oreDict : (String[]) component.metas.get(i).get(2))
					registerRecipe(new RecipeLathe(new RecipeItem(new OreStack("wire" + oreDict, 2)),
							new RecipeItem(stack), 1, 250, LatheModes.SCREW));
		}
		...
		for (int i = 0; i < ((BlockComponent) BlockManager.Component_Metal_Blocks.block()).metas.size(); i++) {
			...
			if (((BlockComponent) BlockManager.Component_Metal_Blocks.block()).metas.get(i).get(2) != null)
				for (String oreDict : (String[]) ((BlockComponent) BlockManager.Component_Metal_Blocks.block()).metas
						.get(i).get(2)) {
					if (OreDictionary.getOres("ingot" + oreDict) != null
							&& !OreDictionary.getOres("ingot" + oreDict).isEmpty()) {
		...
		addShapelessRecipe(new ItemStack(ItemManager.Component_Rods.item()), "cobblestone", "cobblestone",
				"cobblestone");

		addShapedRecipe(new ItemStack(ItemManager.Component_Saw_Blades.item()), " + ", "+-+", " + ", '+',
				new ItemStack(Component_Rods.item()), '-', "cobblestone");

		addShapedRecipe(new ItemStack(ItemManager.Component_Gears.item()), " + ", "+-+", " + ", '+', "plateStone", '-',
				"cobblestone");
	}
