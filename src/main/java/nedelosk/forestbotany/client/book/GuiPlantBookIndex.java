...
public class GuiPlantBookIndex extends GuiPlantBook {
	
	List<BookPlantEntry> entriesToDisplay = new ArrayList();
	String category;
	
	public GuiPlantBookIndex(GuiBook parent, String category, BookData bookData, GameProfile player, World world) {
	...
	public void updateIndex()
	{
		...
		for(int i = page * 12; i < (page + 1) * 12; i++) {
			...
			BookPlantEntry entry = i >= entriesToDisplay.size() ? null : entriesToDisplay.get(i);
			if(entry != null) {
	...
	public void drawScreen(int arg0, int arg1, float arg2) {
		...
		RenderUtils.glDrawScaledString(Minecraft.getMinecraft().fontRenderer,EnumChatFormatting.ITALIC + (StatCollector.translateToLocal(bookData.getCategoryName(category))), this.left +  -18, this.top + 10, 0.8F, Color.gray.getRGB());
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		...
		{
			if(parent instanceof GuiPlantBookIndex)
			{
			mc.displayGuiScreen(new GuiPlantBookIndex(this, ((GuiButtonBookmark) par1GuiButton).category, this.bookData, player, world));
			}
		}
	...
	void openEntry(int index) {
		...
		BookPlantEntry entry = entriesToDisplay.get(index);
		...
		mc.displayGuiScreen(new GuiPlantBookEntry(this, entry, bookData, player, world));
	}
