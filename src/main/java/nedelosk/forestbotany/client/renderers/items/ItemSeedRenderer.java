...
public class ItemSeedRenderer implements IItemRenderer {
	...
	public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        ...
        if(!item.getTagCompound().getCompoundTag("Crop").getBoolean("IsCrop"))
        {
        	
        int icons = SeedFile.values()[((ItemSeed)item.getItem()).getSpecies(item).getSeedType().ordinal()].getRenderPasses()[item.getTagCompound().getCompoundTag("Crop").getInteger("GrowthStage")];
        ...
        else
        {
			...
			{
            GL11.glPushMatrix();
            GL11.glTranslatef(0.0F - 2, 0.0F + 3, -3.0F + 0.0F);
            ...
            RenderUtils.bindItemTexture();
					IIcon iconPlant = ((ItemPlant)item.getItem()).getPlantIcon(item, 0);
