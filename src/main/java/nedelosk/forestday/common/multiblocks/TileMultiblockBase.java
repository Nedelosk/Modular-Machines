    		...
    		if(master != null && worldObj.getTileEntity(master.getXCoord(), master.getYCoord(),  master.getZCoord()) != null && worldObj.getTileEntity(master.getXCoord(), master.getYCoord(), master.getZCoord()) == master)
    		{
    			...
    			if(isMaster)
    				if(multiblock != null)
    				{
    					...
    					nbt.setInteger("Pattern", ForestDayApi.getMutiblockPatterns(multiblock.getMultiblockName()).indexOf(pattern));
    	...
    	if(nbt.hasKey("masterXCoord") || nbt.hasKey("masterYCoord") || nbt.hasKey("masterZCoord"))
    	{
    		...
    		if(isMaster)
    		{
    			if(nbt.hasKey("Multiblock"))
    			{
    				...
    				pattern = ForestDayApi.getMutiblockPatterns(multiblock.getMultiblockName()).get(nbt.getInteger("Pattern"));
    			}
    		}
    	}
    	else if(master != null)
