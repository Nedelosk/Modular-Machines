...
public class BlockMachines extends BlockContainerForest {
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		try {
			...
			FMLLog.log(Level.ERROR, e, "Caught an exception during TileEntity registration in Forestday:BlockMachines "
					+ tiles[meta].getName());
	...
	public void registerBlockIcons(IIconRegister IIconRegister) {
		try {
			...
			for (int i = 0; i < tiles.length; i++) {
				for (int a = 0; a < 2; a++) {
					for (int r = 0; r < 6; r++) {
						icons[i][a][r] = IIconRegister.registerIcon(
								"forestday:machines/" + tiles[0].getConstructor().newInstance().getMachineName() + "/"
										+ getFileFromTier(i) + "/" + getNameFromSide(r) + ((a == 1) ? "_active" : ""));
					}
				}
			}
		} catch (Exception e) {
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int blockSide) {
		...
		if (((TileMachineBase) world.getTileEntity(x, y, z)).isWorking) {
			return icons[world.getBlockMetadata(x, y,
					z)][1][ClientProxy.sideAndFacingToSpriteOffset[blockSide][facing]];
		} else {
			return icons[world.getBlockMetadata(x, y,
					z)][0][ClientProxy.sideAndFacingToSpriteOffset[blockSide][facing]];
		}
	}
