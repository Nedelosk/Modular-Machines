...
import nedelosk.forestday.common.machines.mutiblock.MultiblockForestday;
...
public class MultiblockCharcoalKiln extends MultiblockForestday {
	...
	public boolean isPatternBlockValid(int x, int y, int z, char pattern, ITileMultiblock tile) {
		...
		switch (pattern) {
		case 'C':
            if (block != FBlocks.Multiblock_Charcoal_Kiln.block() && !(baseTile instanceof TileCharcoalKiln))
            {
                return false;
            }
            break;
	...
	public void updateServer(ITileMultiblock tile) {
		...
		if(base.burnTime >= base.burnTimeTotal)
		{
			...
			base.getWorldObj().setBlock(base.xCoord, base.yCoord - 1, base.zCoord, FBlocks.Multiblock_Charcoal_Kiln.block(), 1, 2);
